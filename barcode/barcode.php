 ...
 */
	...
	for ( $position = 1 ; $position <= strlen($code_string); $position++ ) {
		...
		if ( strtolower($orientation) == "horizontal" )
			imagefilledrectangle( $image, $location, 0, $cur_size, $img_height-35, ($position % 2 == 0 ? $white : $black) );
		else
			imagefilledrectangle( $image, 0, $location, $img_width, $cur_size, ($position % 2 == 0 ? $white : $black) );
		$location = $cur_size;
	}
	// Draw barcode to the screen
	$x = imagestring($image, 10, 35, 65, $text, $black); //(image,font size,x,y,string todisplay,color of string) orig:10,35,80 --->height:100
