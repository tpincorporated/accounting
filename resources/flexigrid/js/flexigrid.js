		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function (i, row) {
						...
						if (row[p.rowAttributes]) {     /*** KAYE'S ADDITIONAL CODE **/
							for(var key in row[p.rowAttributes]){   /*** KAYE'S ADDITIONAL CODE **/
								if(key.toLowerCase()=='class'){
									jtr.addClass(row[p.rowAttributes][key]);
								}
								else{
									jtr.attr(key, row[p.rowAttributes][key]); /*** KAYE'S ADDITIONAL CODE **/
								}
							}							/*** KAYE'S ADDITIONAL CODE **/
						}								/*** KAYE'S ADDITIONAL CODE **/
			...
			addRowProp: function () {
				$('tbody tr', g.bDiv).on('click', function (e) {
					...
					if ( ! p.disableSelect ){  /** KAYE'S CODE - only this line**/
						$(this).toggleClass('trSelected');
						if (p.singleSelect && ! g.multisel) {
							$(this).siblings().removeClass('trSelected');
						}
					}	/** KAYE'S CODE - only this line**/
					/*
					$(this).toggleClass('trSelected');
					if (p.singleSelect && ! g.multisel) {
						$(this).siblings().removeClass('trSelected');
