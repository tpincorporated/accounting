$(document).off('click','.editcategorybtn').on('click','.editcategorybtn',function(){
	...
	$(modal+' .categorymodalid').val($(this).attr('rowid'));
});

/***************************** SAVE | EDIT **************************************************/
$(document).off('click','.categorymodal-savebtn:not(".disabled")').on('click','.categorymodal-savebtn:not(".disabled")',function(){
	...
	if(modal=='#editcategorymodal'){
		id = $(modal+' .categorymodalid').val();
	...
	if(code==''){
		...
		$.post(server+'category.php',{categorySaveEdit:'j$isnDPo#P3sll3p23a3!@3kzlsO!mslo#k@',source:source,id:id,code:code,desc:desc,statusflag:statusflag},function(data){
				if(data.trim()=='success'){
					$(modal).modal('hide');
					$(modal).on('hidden.bs.modal',function(){
						
						$('#categorytable').flexOptions({
