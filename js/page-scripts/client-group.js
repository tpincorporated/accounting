$(document).off('click','.editclientgroupbtn').on('click','.editclientgroupbtn',function(){
	var modal = "#editclientgroupmodal";
	...
	$(modal+' .clientgroupmodalid').val($(this).attr('rowid'));
});

/***************************** SAVE | EDIT **************************************************/
$(document).off('click','.clientgroupmodal-savebtn:not(".disabled")').on('click','.clientgroupmodal-savebtn:not(".disabled")',function(){
	...
	if(modal=='#editclientgroupmodal'){
		id = $(modal+' .clientgroupmodalid').val();
	...
	if(code==''){
		...
		$.post(server+'client-group.php',{clientgroupSaveEdit:'j$isnDPo#P3sll3p23a3!@3kzlsO!mslo#k@',source:source,id:id,code:code,desc:desc,statusflag:statusflag},function(data){
				if(data.trim()=='success'){
					$(modal).modal('hide');
					$(modal).on('hidden.bs.modal',function(){
						
						$('#clientgrouptable').flexOptions({
